...
from .phenotype import ElectronRecord, GenomePhenotype, read_phenotype, read_tables
...
# Per-organism phenotypes and electron donors / acceptors; shown in the report when present.
DEFAULT_PHENOTYPE = Path("data") / "electron" / "phenotype_data.tsv"
# Reviewed tables that turn phenotype_data.tsv values into report categories.
DEFAULT_PHENOTYPE_TABLES = Path("config")

Phenotype = Tuple[Optional[Dict[str, GenomePhenotype]], Optional[Dict[str, List[ElectronRecord]]]]
...
def _load_phenotype(args: argparse.Namespace, results: Sequence[GenomeResult]) -> Phenotype:
    if args.no_phenotype:
        return None, None
    path = args.phenotype
    if path is None:
        if not DEFAULT_PHENOTYPE.is_file():
            logger.info("no phenotype data at %s; reports are written without phenotypes / categories", DEFAULT_PHENOTYPE)
            return None, None
        path = DEFAULT_PHENOTYPE
    elif not path.is_file():
        raise ValueError(f"phenotype data not found: {path}")
    logger.info("reading phenotypes and electron donors / acceptors from %s", path)
    phenotypes, electron = read_phenotype(path, read_tables(args.phenotype_tables))
    scanned = {result.genome_id for result in results if result.is_valid}
    logger.info("phenotype data: %d genome(s), %d of the scanned genomes", len(phenotypes), len(scanned & set(phenotypes)))
    return (
        {genome_id: value for genome_id, value in phenotypes.items() if genome_id in scanned},
        {genome_id: rows for genome_id, rows in electron.items() if genome_id in scanned},
    )
...
def cmd_scan(args: argparse.Namespace) -> int:
    ...
    phenotypes, electron = _load_phenotype(args, results)
    run_info = {
        ...
        "phenotype": str(args.phenotype or DEFAULT_PHENOTYPE) if phenotypes is not None else None,
        "n_genome_dirs": len(results),
        "n_genomes_analysed": sum(result.is_valid for result in results),
    }
    paths = write_scan_outputs(args.output_dir, entries, results, summaries, run_info, taxonomy, electron, phenotypes)
    if not args.no_html:
        reports = write_reports(
            args.output_dir, entries, results, run_info, _report_titles(args), taxonomy, electron, phenotypes
        )
...
def cmd_render_html(args: argparse.Namespace) -> int:
    ...
    if not args.no_taxonomy and args.taxonomy_dir is None and taxonomy_file.is_file():
        ...
        taxonomy = _load_taxonomy(args, results)
    phenotype_file = args.results_dir / PHENOTYPE_FILE
    electron_file = args.results_dir / ELECTRON_CATEGORIES_FILE
    if not args.no_phenotype and args.phenotype is None and phenotype_file.is_file() and electron_file.is_file():
        phenotypes, electron = read_phenotypes(phenotype_file), read_electron_categories(electron_file)
    else:
        phenotypes, electron = _load_phenotype(args, results)
    output_dir = args.output_dir or args.results_dir
    reports = write_reports(
        output_dir, entries, results, read_run_info(args.results_dir), _report_titles(args), taxonomy, electron, phenotypes
    )
...
def _add_phenotype_arguments(parser: argparse.ArgumentParser, default_help: str) -> None:
    parser.add_argument("--phenotype", type=Path,
                        help=f"phenotype_data.tsv for the phenotype and donor / acceptor category columns (default: {default_help})")
    parser.add_argument("--phenotype-tables", type=Path, default=DEFAULT_PHENOTYPE_TABLES,
                        help="directory with electron_compound_categories.tsv, phenotype_corrections.tsv and "
                             f"habitat_classes.tsv (default: {DEFAULT_PHENOTYPE_TABLES})")
    parser.add_argument("--no-phenotype", action="store_true", help="do not add phenotype / category columns")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    _add_phenotype_arguments(p, f"{DEFAULT_PHENOTYPE} when present")
    ...
    _add_phenotype_arguments(p, f"<results-dir>/{PHENOTYPE_FILE}, else {DEFAULT_PHENOTYPE} when present")
    _add_title_arguments(p)
